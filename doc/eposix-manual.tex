...
\useURL
  [vestring]
  [http://groups.yahoo.com/group/visual-eiffel/message/673]
...
This software is licensed under the \index{Eiffel Forum Freeware
License}Eiffel Forum Freeware License, version 2. This
\index{license}license can be found in the \MYfilename{forum.txt}
file. Basically this license allows you to do anything with it,
i.e. use it for commercial or \index{Open Source}Open Source software
without restrictions. But don't sue me if something goes wrong. And
...
\startitemize[n]
\item \eposix\ requires Gobo release 3.4 or higher. You can download
...
Only the Microsoft supplied library did work, i.e. link, with
\index{VisualEiffel}VisualEiffel:
...
If you use \index{ISE Eiffel 5.7}ISE Eiffel 5.7, you can use the
supplied \MYfilename{eposix.ecf} which will make inclusion of eposix
in your projects much easier. However, the library path when you use
the \MYfilename{eposix.ecf} is slightly different.

After generation of \MYfilename{libeposix}, see
\in{section}[sec:libeposix], do this on Unix:
...
\subsection{Visual Eiffel}

\eposix\ has been tested with ObjectTool's free
\index{VisualEiffel}VisualEiffel 5.0b for Linux. VisualEiffel 4.1
might still work but is no longer tested.

Follow these steps to compile with VisualEiffel 5 on Windows:
\startitemize[n]
\item Make sure the \index{VE_BIN}VE_BIN environment variable is set
  to the Bin directory in the VisualEiffel subdirectory. On my system it
  is set to \filename{M:/Program Files/ObjectTools/VisualEiffel/Bin}.
\item Create the \MYfilename{libeposix_ve_msc.lib} library using the
  Microsoft Visual C compiler:
 \starttyping
makelib -ve -msc
 \stoptyping
\item Use \type{gexace} to generate an {\tt .esd} file.
\item Make sure to set the linker supplier option to Microsoft in your
  \MYfilename{system.xace} file! So an option like this should be present:
\starttyping
...
The latest version of \eposix\ was tested with kernel 2.6.20.1 and
...
\stoptext
% LocalWords:  writability {Colophon} pid {posix} {posix} BON VisualEiffel Gobo
