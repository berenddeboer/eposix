...
feature -- Change
	...
	attach_file (a_type, a_subtype, a_file_name: STRING; an_inline: BOOLEAN)
		...
		do
			...
			path.parse (Void)
			if attached multipart_body as b then
				file_part := b.new_part
				...
				if attached path.basename as n then
					if attached file_part.header.content_type as ct then
						ct.set_parameter (once "name", n)
					end
					...
					until
						file.end_of_input
					loop
						if attached file_part.text_body as tb then
							tb.append_string (file.last_string)
						end
						file.read_string (8192)
					end
					file.close
